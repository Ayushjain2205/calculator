...
export default function Calculator() {
  ...
  const formatForDisplay = (value: number | string): string => {
    ...
    // For integers
    return numStr.length > MAX_DISPLAY_LENGTH ? numStr.substring(0, MAX_DISPLAY_LENGTH) : numStr
  }
  ...
  const handleScientificFunction = (func: string) => {
    ...
    try {
      switch (func) {
        case "sin":
          result = shift ? Math.asin(num) * (isRadianMode ? 1 : 180 / Math.PI) : Math.sin(convertToRadians(num))
          break
        case "cos":
          result = shift ? Math.acos(num) * (isRadianMode ? 1 : 180 / Math.PI) : Math.cos(convertToRadians(num))
          break
        case "tan":
          result = shift ? Math.atan(num) * (isRadianMode ? 1 : 180 / Math.PI) : Math.tan(convertToRadians(num))
          break
        case "ln":
        ...
        case "abs":
          ...
          break
        case "fact":
          if (num < 0 || !Number.isInteger(num) || num > 170) throw new Error("Invalid input")
  ...
  return (
    <div className="relative w-full max-w-[320px] mx-auto bg-[#333333] rounded-[30px] shadow-2xl p-4 pt-6 pb-8 border-t border-[#444444] border-b-[12px] border-b-[#222222] border-r-[6px] border-r-[#222222] border-l-[6px] border-l-[#444444]">
      ...
      <div className="mt-12 mb-6 mx-2 bg-[#c0ceb0] rounded-md p-3 h-[70px] flex items-end justify-end shadow-inner border-2 border-[#222] overflow-hidden">
        <div className="text-[#222] text-4xl font-lcd" style={{ transform: "scaleY(1.05)" }}>
          {display}
          {storedMemory !== 0 && <span className="text-sm absolute top-2 left-3 font-digital">M</span>}
          {isRadianMode && <span className="text-sm absolute top-2 right-3 font-digital">RAD</span>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 gap-2 mt-4">
        ...
        <button
          className={cn(
            "text-white text-xs py-2 px-1 rounded-md bg-[#444] border border-t-[#555] border-l-[#555] border-b-[#222] border-r-[#222] shadow-md active:shadow-inner active:border-t-[#222] active:border-l-[#222] active:border-b-[#555] active:border-r-[#555]",
            shift && "bg-[#555]",
          )}
          onClick={handleShift}
        >
        ...
        <div className="flex justify-center items-center">
          <div className="w-[40px] h-[40px] bg-[#333] rounded-full border-2 border-[#222] flex flex-col justify-center items-center">
            <div className="flex justify-center items-center">
              <div className="w-[10px] h-[10px] text-[#888] flex justify-center items-center">▲</div>
            </div>
            <div className="flex justify-center items-center">
              <div className="w-[10px] h-[10px] text-[#888] flex justify-center items-center">◀</div>
              <div className="w-[10px] h-[10px] text-[#888] flex justify-center items-center">▶</div>
        ...
        <button
          className="text-white text-xs py-2 px-1 rounded-md bg-[#444] border border-t-[#555] border-l-[#555] border-b-[#222] border-r-[#222] shadow-md active:shadow-inner active:border-t-[#222] active:border-l-[#222] active:border-b-[#555] active:border-r-[#555]"
        ...
        {/* Row 2 */}
        <button
          className="text-white text-xs py-2 px-1 rounded-md bg-[#222] border border-t-[#444] border-l-[#444] border-b-[#111] border-r-[#111] shadow-md active:shadow-inner active:border-t-[#111] active:border-l-[#111] active:border-b-[#444] active:border-r-[#444]"
          onClick={handleShift}
        >
          SHIFT
        </button>
        <button className="text-white text-xs py-2 px-1 rounded-md bg-[#222] border border-t-[#444] border-l-[#444] border-b-[#111] border-r-[#111] shadow-md active:shadow-inner active:border-t-[#111] active:border-l-[#111] active:border-b-[#444] active:border-r-[#444]">
          MODE
        </button>
        <button
          className="text-white text-xs py-2 px-1 rounded-md bg-[#222] border border-t-[#444] border-l-[#444] border-b-[#111] border-r-[#111] shadow-md active:shadow-inner active:border-t-[#111] active:border-l-[#111] active:border-b-[#444] active:border-r-[#444]"
        ...
        <button
          className="text-black text-xl py-2 px-1 rounded-md bg-[#aaa] border border-t-[#ccc] border-l-[#ccc] border-b-[#888] border-r-[#888] shadow-md active:shadow-inner active:border-t-[#888] active:border-l-[#888] active:border-b-[#ccc] active:border-r-[#ccc]"
        ...
        <button
          className="text-white text-sm py-2 px-1 rounded-md bg-[#444] border border-t-[#555] border-l-[#555] border-b-[#222] border-r-[#222] shadow-md active:shadow-inner active:border-t-[#222] active:border-l-[#222] active:border-b-[#555] active:border-r-[#555]"
          onClick={handleBackspace}
        >
          DEL<span className="text-yellow-400 text-xs block">⌫</span>
        </button>
        <button
          className="text-white text-sm py-2 px-1 rounded-md bg-[#444] border border-t-[#555] border-l-[#555] border-b-[#222] border-r-[#222] shadow-md active:shadow-inner active:border  border-l-[#555] border-b-[#222] border-r-[#222] shadow-md active:shadow-inner active:border-t-[#222] active:border-l-[#222] active:border-b-[#555] active:border-r-[#555]"
        ...
        <button
          className="text-white text-xl py-2 px-1 rounded-md bg-[#444] border border-t-[#555] border-l-[#555] border-b-[#222] border-r-[#222] shadow-md active:shadow-inner active:border-t-[#222] active:border-l-[#222] active:border-b-[#555] active:border-r-[#555]"
          onClick={() => handleOperationInput("×")}
        >
          ×
        </button>
        ...
        <button
          className="text-white text-sm py-2 px-1 rounded-md bg-[#aaa] border border-t-[#ccc] border-l-[#ccc] border-b-[#888] border-r-[#888] shadow-md active:shadow-inner active:border-t-[#888] active:border-l-[#888] active:border-b-[#ccc] active:border-r-[#ccc]"
          onClick={handleAns}
        >
          ANS
        </button>
